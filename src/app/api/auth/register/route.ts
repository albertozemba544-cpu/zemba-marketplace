...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, role, full_name, business_name || null, phone_number, email, hashedPassword, momo_provider || null, momo_number || null, approvalStatus, role.toUpperCase(), newsletter_opt_in === false ? 0 : 1);
    
    // Save vendor verification details for sellers
    ...
    if (String(error).includes('UNIQUE')) return NextResponse.json({ error: 'Email or phone number is already registered' }, { status: 409 });
